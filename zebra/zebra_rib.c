...
static const struct {
	...
	[ZEBRA_ROUTE_ALL] = {ZEBRA_ROUTE_ALL, ZEBRA_MAX_DISTANCE_DEFAULT,
			     META_QUEUE_OTHER},
	[ZEBRA_ROUTE_ADJACENCY] = {ZEBRA_ROUTE_ADJACENCY, 0,
				   META_QUEUE_CONNECTED},
 ...
 *  1 -> update
 */
int rib_add_multipath_nhe(afi_t afi, safi_t safi, const struct prefix *p, struct prefix_ipv6 *src_p,
...
int rib_add(afi_t afi, safi_t safi, vrf_id_t vrf_id, int type, unsigned short instance,
	    uint32_t flags, const struct prefix *p, struct prefix_ipv6 *src_p, const struct nexthop *nh,
...
static bool is_valid_host_route(const struct ipaddr *ip)
{
	...
	if (ip->ipa_type != AF_INET && ip->ipa_type != AF_INET6)
		return false;
	if (ip->ipa_type == AF_INET6 &&
	    !is_ipv6_global_unicast(&(ip->ipaddr_v6))) {
		return false;
	} else if ((ip->ipa_type == AF_INET) &&
		   IN_MULTICAST(ip->ipaddr_v4.s_addr)) {
		return false;
	}
		       ...
		       uint32_t flags)
{
	...
	/* Allocate new route_entry structure. */
	re = zebra_rib_route_entry_new(vrf->vrf_id, ZEBRA_ROUTE_ADJACENCY, 0,
				       flags, 0, zvrf->table_id, 0, 0, 0, 0);
	/* Add rib entry */
		       ...
		       uint32_t flags)
{
	...
	rib_delete(afi, SAFI_UNICAST, vrf->vrf_id, ZEBRA_ROUTE_ADJACENCY,
		   0, flags, &p, NULL, &nexthop, 0, zvrf->table_id, 0, 0,
		   false);
