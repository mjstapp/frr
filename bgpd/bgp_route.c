...
#if 0 /* TODO -- remove this table-walking version */
/*
...
next_dest:
	...
	counter++;
	if (counter < bm->peer_clearing_batch_max_dests)
		goto next_dest;
	...
	if (bgp_clearing_batch_dests_present(cinfo)) {
		if (bgp_debug_neighbor_events(NULL))
			zlog_debug("%s: Batch %#x: Rescheduled after processing %d dests",
				   __func__, cinfo->id, counter);

		event_add_event(bm->master, bgp_clear_batch_dests_task, cinfo,
				0, &cinfo->t_sched);
	} else {
		if (bgp_debug_neighbor_events(NULL))
			zlog_debug("%s: Batch %#x: Done after processing %d dests",
				   __func__, cinfo->id, counter);
 ...
 */
static int clear_batch_table_helper(struct bgp_clearing_info *cinfo,
				   ...
				   struct bgp_table *table, bool inner_p)
{
	...
	for ( ; dest; dest = bgp_route_next(dest)) {
		...
		for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = next) {
			...
			/* TODO -- remove debug */
			zlog_debug("%s: dest %pRN: found peer %pBP", __func__,
				   dest->rn, pi->peer);
...
/* Iterate afi/safi combos, after init */
#define FOR_AFI_SAFI(afi, safi) \
	for ( ; (afi) < AFI_MAX; (afi)++) \
...
static int clear_batch_rib_helper(struct bgp_clearing_info *cinfo)
{
	...
	FOR_AFI_SAFI(afi, safi) {
		/* Identify table to be examined: special handling
		 * for some SAFIs
		 */
		if (bgp_debug_neighbor_events(NULL))
			zlog_debug("%s: examining AFI/SAFI %s/%s", __func__,
		...
		if (safi != SAFI_MPLS_VPN && safi != SAFI_ENCAP &&
			...
			ret = walk_batch_table_helper(cinfo, table,
						      false /*inner*/);
			if (ret != 0)
				break;
			...
			for ( ; dest; dest = bgp_route_next(dest)) {
				...
				if (ret != 0) {
					 ...
					 */
					set_clearing_resume_info(cinfo,
								 outer_table,
								 dest, false);
					break;
				}
