...
"""
test_bgp_evpn_vxlan.py: Test RTC feature with EVPN L2VNI and iBGP
"""
...
def setup_module(mod):
    ...
    # For all routers, load the configuration file
    for rname, router in router_list.items():
        router.load_frr_config(
            os.path.join(CWD, "{}_evpn_vxlan/frr.conf".format(rname))
        )
...
def test_pe1_converge_evpn():
    "Wait for PE1 protocol convergence"
    ...
    logger.info("Checking for initial evpn convergence")

    pe1 = tgen.gears["PE1"]
    json_file = "{}/{}_evpn_vxlan/evpn.vni.json".format(CWD, pe1.name)
...
def test_pe2_converge_evpn():
    "Wait for PE2 protocol convergence"
    ...
    pe2 = tgen.gears["PE2"]
    json_file = "{}/{}_evpn_vxlan/evpn.vni.json".format(CWD, pe2.name)
    expected = json.loads(open(json_file).read())
