...
#
# Helper to check ADJ route info from route output
#
def check_route(tor, addr_str):

    # show route on local TOR
    logger.info(
        "Check {} zebra route: {}".format(tor.name, addr_str)
    )

    output = tor.vtysh_cmd("show ip route {} json".format(addr_str))
    ...
    try:
        ...
        return "{}: route {} not found".format(tor.name, addr_str)
